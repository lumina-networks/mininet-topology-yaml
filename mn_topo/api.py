...
def create_dc_topology(filename, data_centres=1, spines=2, leaves=4, hosts=1, controllers=[], interfaces=[]):
    data = table.create_table_topology(data_centres, spines, leaves, hosts)
    data['controllers'] = controllers
    data['interfaces'] = interfaces
    return api.create_topology(filename, **data)